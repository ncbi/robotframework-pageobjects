...
class PageComponentTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        class SearchForm(PageComponent):
            """ A Mocked search form page component
            for testing.
            """
            selectors = {
                # Assure the input is the correct one. There might
                # be >1 on the page, even with the same ID.
                "input el": "xpath=id('search-form')/input[@id='q']",
            }

            def __init__(self, *args, **kwargs):
                super(SearchForm, self).__init__(*args, **kwargs)
                self.input_el = self._element_find("input el", [], {})[0]

            def set_query(self, term):
                self.input_el.send_keys(term)

            def get_query(self):
                return self.input_el.get_attribute("value")


        super(PageComponentTestCase, self).setUp()
        self.set_baseurl_env()

        # Set what page components this page has.
        Page.components = [SearchForm]

        # Mock a few page object methods that use the SearchForm
        # component internally.
        def set_query(Page, term):
            Page._components["SearchForm"].set_query(term)

        def get_query(Page):
            return Page._components["SearchForm"].get_query()

        # Bind those mocked methods to the Page object.
        Page.set_query = set_query
        Page.get_query = get_query

        self.p = Page()
        self.p.uri = "/site/index.html"
        self.p.open()

    def test_can_set_and_get(self):
        self.p.set_query("foo")
        self.assertEquals(self.p.get_query(), "foo")

    def tearDown(self):
        super(PageComponentTestCase, self).tearDown()
        self.p.close()
